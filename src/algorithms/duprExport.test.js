...
describe('buildDUPRRows — TPT mode', () => {
  ...
  it('emits one row per game with the correct sides and scores', () => {
    ...
    expect(rows[0]).toMatchObject({
      ...
      playerB1: 'Chris',
      playerB1DuprId: 'JK01L2',
      playerB2: 'Pat',
      playerB2DuprId: 'MN34O5',
      teamAGame1: 11,
      teamBGame1: 5,
      matchType: 'D',
    });
...
describe('buildDUPRRows — swiss/round-robin mode', () => {
  const teams = {
    red: {
      ...
      name: 'Red',
      players: [
        { name: 'Jane Doe', duprId: 'AB12C3' },
        { name: 'Alex Smith', duprId: 'DE45F6' },
      ],
    },
    blue: {
      id: 'blue',
      name: 'Blue',
      players: [
        { name: 'Sam Lee', duprId: 'GH78I9' },
        { name: 'Chris Park', duprId: 'JK01L2' },
      ],
    },
    ...
    const history = [
      {
        roundNum: 1,
        games: [
          { winnerId: 'red', loserId: 'blue', winnerScore: 11, loserScore: 7, courtNumber: '1' },
        ],
      },
    ];
...
describe('collectTPTPlayerIds / collectSwissTeamIds', () => {
  it('dedupes players appearing across TPT matchups', () => {
    const tptTeams = {
      A: { id: 'A', maleIds: ['m1', 'm2'], femaleId: 'f1' },
      B: { id: 'B', maleIds: ['m3', 'm4'], femaleId: 'f2' },
    };
    const history = [
      { roundNum: 1, tptMatchups: [{ teamAId: 'A', teamBId: 'B', games: [] }] },
      { roundNum: 2, tptMatchups: [{ teamAId: 'A', teamBId: 'B', games: [] }] },
    ];
    expect(collectTPTPlayerIds({ history, tptTeams }).sort()).toEqual([
      'f1',
      'f2',
      'm1',
      'm2',
    ...
    const history = [
      {
        roundNum: 1,
        games: [
          { winnerId: 'red', loserId: 'blue' },
          { winnerId: 'red', loserId: 'green' },
        ],
      },
...
describe('buildDUPRCsv', () => {
  it('produces the exact DUPR header followed by escaped row data', () => {
    ...
    const csv = buildDUPRCsv(rows, {
      eventName: 'Tuesday Night League',
      date: '2025-01-15',
