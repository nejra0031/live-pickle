...
export function mkStandings(
  ids: string[]
): (Standing & { lastPlayedRound: number; lastByeRound: number })[] {
  return ids.map((id) => ({
    ...teamByIdModule(id),
    id,
    wins: 0,
...
export function rerank<T extends Standing>(st: T[], order?: string[], history?: RoundEntry[]): T[] {
  const criteria = order || ['wins', 'scoreDiff'];

  let h2h: Record<string, { wins: number; scoreDiff: number }> | null = null;
  ...
  return [...st].sort((a, b) => {
    ...
    return (a as any).played - (b as any).played;
  });
}
...
export function rebuildStandings(
  ...
  history.forEach((rd, ri) => {
    rd.games.forEach((g) => {
      const w = map[g.winnerId],
        l = map[g.loserId];
      if (!w || !l) return;
      w.wins++;
      w.scoreDiff += g.winnerScore - g.loserScore;
      w.played++;
      w.lastPlayedRound = ri;
      l.losses++;
      l.scoreDiff += g.loserScore - g.winnerScore;
      l.played++;
      l.lastPlayedRound = ri;
    });
    (rd.bye || []).forEach((id) => {
      if (map[id]) map[id].lastByeRound = ri;
    });
  });
