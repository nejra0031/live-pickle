...
import {
  ...
  buildDoublesRRStandings,
  isValidDoublesRRPlayerCount,
  nearestValidDoublesRRPlayerCounts,
} from './doublesRR';
...
describe('generateDoublesRRSchedule', () => {
  ...
  it('pairs every player with every other player exactly once (odd N, with byes)', () => {
    ...
    schedule.forEach((round) => {
      ...
      players.forEach((id) =>
        expect(onCourt.has(id) || round.byePlayerIds.includes(id)).toBe(true)
      );
    });
  });
