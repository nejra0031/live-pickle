...
export const TOURNAMENT_MODES = ['swiss', 'roundrobin', 'tpt', 'doublesrr'] as const;
...
export function getSortTimestamp(t: TournamentSummary): number | null {
  if (t.startTime) {
...
export function isTournamentFull(t: TournamentSummary): boolean {
  const count = t.playerCount || t.teamCount || 0;
  return (t.maxPlayers || 0) > 0 && count >= (t.maxPlayers || 0);
}
...
export function splitAndSortTournaments(tournaments: TournamentSummary[]): {
  ...
  const cmp = (dir: 1 | -1) => (a: TournamentSummary, b: TournamentSummary) => {
    const ta = getSortTimestamp(a),
      tb = getSortTimestamp(b);
...
export function applyLandingFilters(
  { upcoming, finished }: { upcoming: TournamentSummary[]; finished: TournamentSummary[] },
  { enabledModes, hideFull }: { enabledModes: Set<string> | string[]; hideFull: boolean }
): { upcoming: TournamentSummary[]; finished: TournamentSummary[] } {
  ...
  const byMode = (t: TournamentSummary) => !modeFilterActive || modeSet.has(t.mode || '');
