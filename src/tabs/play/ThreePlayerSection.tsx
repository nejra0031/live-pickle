    ...
    return (
      <div
        ...
        style={{
          ...
          borderRadius: 14,
          background: isCurrentRound ? '#fff' : 'rgba(0,0,0,0.02)',
          border: `1px solid ${isCurrentRound ? 'rgba(0,0,0,0.1)' : 'rgba(0,0,0,0.07)'}`,
      ...
      >
        ...
        {games.map((game, gi) => {
          ...
          const pendingResult = stored
            ? {
                winnerId: stored.winnerTeamId,
                loserId: stored.loserTeamId,
                winnerScore: stored.winnerScore,
                loserScore: stored.loserScore,
              }
            : null;
          const sideA = {
            id: teamA.id,
            name: sideLabel(game.sideA, tptPlayers),
            color: teamA.color,
            text: teamA.text,
          };
          const sideB = {
            id: teamB.id,
            name: sideLabel(game.sideB, tptPlayers),
            color: teamB.color,
            text: teamB.text,
          };
            ...
            <div key={key}>
              {pendingResult || (isCurrentRound && canWrite) ? (
                <CourtCard
                  ...
                  onUndo={
                    pendingResult && isCurrentRound && canWrite && onUndoTPTResult
                      ? () => onUndoTPTResult(ri, mi, gi)
                      : undefined
                  }
                />
              ) : (
      ...
      {tptSchedule.length === 0 ? (
        ...
        >
          ...
          <p style={{ color: '#64748b', fontSize: 13, margin: 0 }}>
            {isAdmin
              ? 'Add at least 2 teams in Tournament Settings to generate the tournament schedule.'
          ...
          {viewedRound.byeTeamId && tptTeams[viewedRound.byeTeamId] && (
            ...
            >
              ...
              <span
                ...
                style={{
                  background: tptTeams[viewedRound.byeTeamId].color,
                  color: tptTeams[viewedRound.byeTeamId].text,
