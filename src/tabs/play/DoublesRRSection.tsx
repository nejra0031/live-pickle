...
export default function DoublesRRSection({
  ...
  const renderCourtCard = (court: any, ci: number, ri: number, isCurrentRound: boolean) => {
    ...
    const pendingResult = stored
      ? {
          winnerId: stored.winnerIds.join('|'),
          loserId: stored.loserIds.join('|'),
      ...
      <div key={ci}>
        {pendingResult || (isCurrentRound && canWrite) ? (
          <CourtCard
            ...
            onUndo={
              pendingResult && isCurrentRound && canWrite && onUndoDoublesRRResult
                ? () => onUndoDoublesRRResult(ri, ci)
                : undefined
            }
          />
        ) : (
        ...
        >
          ...
          <p style={{ color: '#64748b', fontSize: 13, margin: 0 }}>
            {isAdmin
              ...
              : 'Waiting for the organizer to add players.'}
          </p>
          {isAdmin && (
            <button
