...
describe('reindexPendingAfterRemoval', () => {
  it('drops the removed index and shifts higher ones down', () => {
    ...
    expect(reindexPendingAfterRemoval(pending, 'court_', [1])).toEqual({
      court_0: 'a',
      court_1: 'c',
    });
  });
  ...
  it('preserves keys outside the prefix', () => {
    ...
    expect(reindexPendingAfterRemoval(pending, 'court_', [0])).toEqual({
      live_0: 'L',
      rr_0_1: 'R',
    });
  });
  ...
  it('handles the live_ prefix independently of court_', () => {
    ...
    expect(reindexPendingAfterRemoval(pending, 'live_', [0])).toEqual({
      court_0: 'a',
      live_0: 'y',
      live_1: 'z',
    });
  ...
  it('shifts cumulatively when several indices are removed', () => {
    ...
    expect(reindexPendingAfterRemoval(pending, 'court_', new Set([1, 3]))).toEqual({
      court_0: 'a',
      court_1: 'c',
      court_2: 'e',
    });
  ...
  it('accepts an array or a Set equivalently', () => {
    ...
    expect(reindexPendingAfterRemoval(pending, 'court_', [0])).toEqual(
      reindexPendingAfterRemoval(pending, 'court_', new Set([0]))
    );
  });
