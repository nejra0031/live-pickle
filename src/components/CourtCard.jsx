...
export default function CourtCard({ courtLabel, teams, onResult, pendingResult, onEdit, onRemove, onUndo }) {
    ...
    return (
      <div className="rounded-2xl"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <span style={{ color: '#16a34a', fontSize: 'clamp(16px,4vw,22px)', flexShrink: 0 }}>✓</span>
          <span style={{ color: '#0f4c75', fontWeight: 800, fontSize: 'clamp(10px,2.5vw,13px)', textTransform: 'uppercase', letterSpacing: '0.08em', flexShrink: 0 }}>{courtLabel}</span>
          {winner && loser && (
            <div className="flex items-center gap-1 flex-1 flex-wrap" style={{ minWidth: 0 }}>
              <span style={{ color: winner.color, fontWeight: 800, fontSize: 'clamp(12px,3vw,15px)', whiteSpace: 'nowrap' }}>{winner.name}</span>
              <span style={{ fontWeight: 900, color: '#1e293b', fontSize: 'clamp(13px,3vw,16px)' }}>{pendingResult.winnerScore}</span>
              <span style={{ color: '#94a3b8', fontSize: 'clamp(12px,3vw,15px)' }}>–</span>
              <span style={{ fontWeight: 900, color: '#1e293b', fontSize: 'clamp(13px,3vw,16px)' }}>{pendingResult.loserScore}</span>
              <span style={{ color: loser.color, fontWeight: 800, fontSize: 'clamp(12px,3vw,15px)', whiteSpace: 'nowrap' }}>{loser.name}</span>
            </div>
          )}
          {onUndo && (
            <button onClick={onUndo} style={{ fontSize: 11, padding: '2px 8px', borderRadius: 6, cursor: 'pointer', background: 'rgba(220,38,38,0.08)', color: '#dc2626', border: '1px solid rgba(220,38,38,0.2)', flexShrink: 0, marginLeft: 'auto' }}>
              ✕ Undo
            </button>
          )}
        </div>
  ...
  return (
    <div className="rounded-2xl flex flex-col"
      ...
      <div className="flex items-center gap-2">
        ...
        {onRemove && (
          <button onClick={onRemove}
            style={{ fontSize: 'clamp(10px,2vw,12px)', padding: '2px 8px', borderRadius: 6, fontWeight: 600, cursor: 'pointer', background: 'rgba(220,38,38,0.08)', color: '#dc2626', border: '1px solid rgba(220,38,38,0.25)', flexShrink: 0 }}>
            ✕ Remove
          </button>
        )}
      </div>
      ...
      <button onClick={confirm} disabled={!valid}
        style={{ width: '100%', padding: 'clamp(10px,2.5vw,14px)', borderRadius: 12, fontWeight: 800, fontSize: 'clamp(13px,3.5vw,18px)', cursor: valid ? 'pointer' : 'not-allowed', background: valid ? 'linear-gradient(90deg,#0f4c75,#1a6fa8)' : 'rgba(0,0,0,0.05)', color: valid ? '#fff' : '#94a3b8', border: valid ? 'none' : '1px solid rgba(0,0,0,0.07)' }}>
