...
export default function MatchupVsBox({ courtLabel, teamA, teamB, compact = false, dimA = false, dimB = false, headerExtra = null }) {
  const boxPadding = compact ? 'clamp(10px,2.5vw,16px)' : 'clamp(14px,3.5vw,24px) clamp(10px,2.5vw,16px)';
  ...
  const Side = ({ team, dim }) => (
    <div className={`flex-1 flex items-center justify-center ${borderRadius}`}
      style={{ padding: boxPadding, background: team.chipBackground ?? team.color, border: `2px solid ${team.color}`, opacity: dim ? 0.4 : 1 }}>
      <span className="font-black text-center leading-tight" style={{ fontSize: nameFontSize, color: team.text }}>{team.name}</span>
    </div>
  );
    ...
    <div className="flex flex-col" style={{ gap: 'clamp(6px,1.5vw,10px)' }}>
      {(courtLabel || headerExtra) && (
        <div className="flex items-center gap-2">
          {courtLabel && <p style={{ fontSize: 'clamp(10px,2.5vw,13px)', color: '#0f4c75', fontWeight: 800, margin: 0, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{courtLabel}</p>}
          {headerExtra}
        </div>
      )}
