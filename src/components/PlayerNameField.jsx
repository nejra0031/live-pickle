...
// Paired "Name" + "DUPR ID" inputs (+ optional "Nickname") with name- and
// DUPR-ID-based autocomplete sourced from a cross-tournament known-players
// registry. Selecting a suggestion fills all provided fields. Pass a
// `nickname` prop (even an empty string) to show the third input.
    ...
    <div ref={wrapRef} style={{ display: 'flex', flexDirection: 'column', gap: 4, position: 'relative' }}>
      ...
      {showNickname && (
        <input
          ...
          onChange={e => emit({ nickname: e.target.value })}
          style={inputStyle}
        />
      )}
    </div>
