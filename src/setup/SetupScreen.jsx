...
export default function SetupScreen({ onStart, onStartTPT, onStartDoublesRR }) {
  ...
  const handleCreate = () => {
    if (format === 'trio') {
      ...
      onStart([], [], DEFAULT_COURTS, DEFAULT_TIMER_SECS, 'Tournament', 0, EMPTY_EVENT_DETAILS, startMode);
    }
  };
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-2xl p-6 flex flex-col gap-3" style={{ background: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.7)', backdropFilter: 'blur(8px)' }}>
        <div>
          ...
          <p className="text-slate-500 text-xs">Pick how teams are formed and how rounds are scheduled. Teams/players, courts, location, and start time are all set up afterwards in ⚙️ Tournament Settings.</p>
        </div>
        <FormatSelector value={selectedFormatOptionId} onChange={selectFormatOption} />
      </div>

      <button onClick={handleCreate} className="w-full py-3 rounded-xl font-bold text-base btn-blue">
        Create Tournament 🚀
      </button>
    </div>
  );
