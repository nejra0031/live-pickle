...
export default function SetupScreen({ onStart, onStartTPT, onStartDoublesRR }) {
  ...
  const [playerDraftError, setPlayerDraftError] = useState('');
  ...
  // Names already present in the current roster (flat list or team slots),
  // keyed the same way as useKnownPlayers (`name.trim().toLowerCase()`) — used
  // to hide already-added players from PlayerNameField's autocomplete suggestions.
  const playerKey = n => n.trim().toLowerCase();
  const addedPlayerKeys = new Set(
    (isFlat ? flatPlayers.map(p => p.name) : teams.flatMap(t => t.players.map(s => s.name)))
      .map(playerKey).filter(Boolean)
  );
  ...
  const addFlatPlayer = () => {
    ...
    if (!name) return;
    if (flatPlayers.some(p => playerKey(p.name) === playerKey(name))) {
      setPlayerDraftError('This player has already been added.');
      return;
    }
          ...
          <div>
            ...
            <div className="flex flex-col sm:flex-row gap-2 mb-3 items-start">
              <div style={{ flex: 1, minWidth: 0 }}>
                <PlayerNameField name={playerDraft.name} duprId={playerDraft.duprId} nickname={playerDraft.nickname} knownPlayers={knownPlayers} excludeKeys={addedPlayerKeys}
                  onChange={val => { setPlayerDraft(d => ({ name: val.name, duprId: val.duprId, nickname: val.nickname ?? d.nickname })); setPlayerDraftError(''); }}
            ...
            {playerDraftError && <p className="text-amber-600 text-xs mb-2">{playerDraftError}</p>}
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: `repeat(${team.players.length}, 1fr)`, gap: 8 }}>
                      {team.players.map((slot, sIdx) => (
                        <div key={slot.id}>
                          ...
                          <PlayerNameField name={slot.name} duprId={slot.duprId} nickname={slot.nickname} knownPlayers={knownPlayers} excludeKeys={addedPlayerKeys}
