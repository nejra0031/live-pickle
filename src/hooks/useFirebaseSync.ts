...
export function useFirebaseSync({
  ...
  useEffect(() => {
    const subs = ROLES.map((r) => {
      const pinsRef = repo.configRef(r.firebasePinsPath);
      onValue(
        pinsRef,
        (snap) => {
          const val = snap.val() || {};
  ...
  useEffect(() => {
    repo
      ...
      .then((snap) => {
        const d = snap.val();
        if (d)
          setBackupRoundNums(new Set(Object.keys(d).map((k) => Number(k.replace('round_', '')))));
      })
      .catch(() => {});
  }, [repo]);
  ...
  useEffect(() => {
    ...
    onValue(r, (snap) => {
      ...
      if (data.phase !== 'play') {
        onSnapshotRef.current({ phase: 'waiting' });
        return;
      }
      const validationError = validateSnapshot(data);
      if (validationError) {
        onFirebaseErrorRef.current(
          `Data integrity error: ${validationError}. Do not enter results — contact the organiser.`
        );
        return;
      }
      if (
        ...
        data._tournamentId !== tournamentIdRef.current &&
  ...
  useEffect(() => {
    ...
    const listenPresRef = repo.presenceRef();
    onValue(listenPresRef, (snap) => {
      const d = snap.val() || {};
      const e = Object.values(d) as any[];
      const cutoff = Date.now() - STALE_MS;
      const live = e.filter((x) => x?.lastSeen >= cutoff);
      const counts: Record<string, number> = {
        viewer: live.filter((x) => !x?.role || x?.role === 'viewer').length,
      };
      ROLES.forEach((r) => {
        counts[r.id] = live.filter((x) => x?.role === r.id).length;
      });
      setPresence(counts);
    });
    ...
    onValue(connRef, (snap) => {
      ...
      clearInterval(presHeartbeatRef.current ?? undefined);
      presHeartbeatRef.current = setInterval(() => {
        if (myPresRef.current)
          fbUpdate(myPresRef.current, { lastSeen: Date.now() }).catch(() => {});
      }, HEARTBEAT_MS);
    });
    return () => {
