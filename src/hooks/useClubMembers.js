...
export function useClubMembers(clubId) {
  ...
  const addMember = useCallback(async (name, duprId, nickname) => {
    const trimmedName = (name || '').trim();
    if (!trimmedName) return;
    const uid = memberSlug(trimmedName);
    const profile = { id: uid, name: trimmedName, duprId: (duprId || '').trim(), nickname: (nickname || '').trim(), createdAt: Date.now() };
    await update(userRef(uid), { profile, clubs: { [clubId]: true } });
    // Also save to known_players so they appear in tournament setup autocomplete
