...
import {
  get, clubsIndexRef, clubInfoRef, clubTournamentsRef,
  ensureClubsIndexBootstrapped, deleteTournament as fbDeleteTournament,
} from '../firebase';
...
export function useAllClubs() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const results = await Promise.all(clubIds.map(async (clubId) => {
  ...
  const deleteTournament = useCallback(async (clubId, tid) => {
