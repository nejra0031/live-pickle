...
function modalReducer(state, action) {
  switch (action.type) {
    case 'OPEN':  return { open: action.modal, data: action.data ?? null };
    case 'CLOSE': return { open: null, data: null };
...
export function useModalState() {
  ...
  const openModal  = useCallback((modal, data = null) => dispatch({ type: 'OPEN', modal, data }), []);
