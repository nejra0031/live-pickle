...
export default function ManageCourtsModal({ courtNumbers, socialCourts = [], rrCourtCount = 0, onSave, onClose }) {
  ...
  const remove = (i) => { setLocal(p => p.filter((_, j) => j !== i)); setLocalSocial(p => p.filter((_, j) => j !== i)); };
  const toggleSocial = (i) => setLocalSocial(p => p.map((v, j) => j === i ? !v : v));
  const addCourt = () => {
  ...
  const rrWarning = rrCourtCount > 0 && local.filter((_, i) => !localSocial[i]).length < rrCourtCount;

  const handleSave = () => {
    if (!valid) return;
    const trimmed = local.map(v => v.trim());
    const newSocialCourts = trimmed.filter((_, i) => localSocial[i]);
    onSave(trimmed, newSocialCourts);
  };
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="rounded-2xl p-5 w-full max-w-xs flex flex-col gap-4 my-4 modal-box" onClick={e => e.stopPropagation()}>
        ...
        <p className="text-xs text-slate-500">Rename, add, or remove courts. Social courts are excluded from round generation.</p>
        {rrWarning && (
          <div className="notice-amber">⚠️ Round Robin is active with {rrCourtCount} court{rrCourtCount !== 1 ? 's' : ''} — reducing competitive courts below that may break the schedule.</div>
        )}
        ...
        <div className="flex gap-2 pt-1">
          ...
          <button onClick={handleSave} disabled={!valid} className="flex-1 py-2 rounded-xl text-sm font-bold btn-indigo">Save</button>
        </div>
      </div>
    </div>
  );
