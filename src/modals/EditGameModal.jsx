...
export default function EditGameModal({ game, roundEntry, allTeamIds, label, onSave, onClose, scoreOnly = false }) {
  ...
  const teamsChanged = !scoreOnly && (teamAId !== game.winnerId || teamBId !== game.loserId);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="rounded-2xl p-5 w-full max-w-sm flex flex-col gap-4 my-4 modal-box" onClick={e => e.stopPropagation()}>
          ...
          <>
            <div className="flex items-center gap-3">
              <div className="flex-1 flex items-center gap-2 rounded-xl px-3 py-2.5"
                ...
                <span className="font-bold text-sm flex-1" style={{ color: '#e2e8f0' }}>{teamA?.name || '—'}</span>
              </div>
        ...
        {!scoreOnly && !teamsValid && teamAId === teamBId && <p className="text-xs text-amber-400 text-center">Teams must be different.</p>}
