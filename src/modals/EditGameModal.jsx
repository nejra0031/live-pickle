  ...
  return (
    <div className="flex items-center gap-3">
      <div className="flex-1 flex items-center gap-2 rounded-xl px-3 py-2.5"
        style={{ background: side?.color ? `${side.color}22` : 'rgba(255,255,255,0.04)', border: `1.5px solid ${side?.color || 'rgba(255,255,255,0.1)'}` }}>
        {side?.color && <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: side.color }} />}
        <span className="font-bold text-sm flex-1" style={{ color: '#e2e8f0' }}>{side?.label ?? '—'}</span>
      </div>
...
export default function EditGameModal({ label, sideA, sideB, scoreA0, scoreB0, courtNum0, teamPicker, onSave, onClose }) {
  ...
  const displayA = editable ? { ...teamPicker.getTeam(teamAId), label: teamPicker.formatLabel(teamAId) } : sideA;
  const displayB = editable ? { ...teamPicker.getTeam(teamBId), label: teamPicker.formatLabel(teamBId) } : sideB;
  const winnerLabel = aWins ? displayA.label : displayB.label;

  const save = () => {
    if (!valid) return;
    onSave({
      scoreA: Number(scoreA), scoreB: Number(scoreB), aWins,
      ...(editable ? { teamAId, teamBId, courtNum: courtNum.trim() } : {}),
    });
  };

  const chipBtn = (id, active, onClick) => {
    const t = teamPicker.getTeam(id); if (!t) return null;
    return (
  ...
  return (
    <div className="modal-overlay">
      <div className="rounded-2xl p-5 w-full max-w-sm flex flex-col gap-4 my-4 modal-box" onClick={e => e.stopPropagation()}>
        <div className="text-sm font-bold text-indigo-300 uppercase tracking-widest">{label} — Edit{editable ? '' : ' scores'}</div>
          ...
          <>
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pb-1">
                {teamPicker.allTeamIds.map(id => chipBtn(id, teamAId === id, () => { setTeamAId(id); if (teamBId === id) setTeamBId(teamAId); }))}
              </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-wrap gap-1.5 max-h-24 overflow-y-auto pb-1">
                {teamPicker.allTeamIds.map(id => chipBtn(id, teamBId === id, () => { setTeamBId(id); if (teamAId === id) setTeamAId(teamBId); }))}
              </div>
        ...
        {valid && <p className="text-xs text-green-400 text-center">→ {winnerLabel} wins {newWinnerScore}–{newLoserScore}</p>}
        {!scoresValid && scoreA !== '' && scoreB !== '' && <p className="text-xs text-amber-400 text-center">Scores can't be equal.</p>}
        {editable && !teamsValid && teamAId === teamBId && <p className="text-xs text-amber-400 text-center">Teams must be different.</p>}
